<div align="center">
  <h1>🍌 Bobanana 5.0</h1>
  <p><b>你说话，AI 干活</b> — 基于 Reasonix 的可编程 AI 开发管线</p>
  <p>
...
Bobanana 是一个 **AI 团队模拟器**。你说一个目标，它像真实团队一样分工协作：

| 角色 | 对应现实 | 干什么 |
|------|---------|--------|
| 👔 Boss | 产品负责人 | 听需求、分任务 |
| 🏗️ 架构师 | 技术 lead | 画图纸、拆模块 |
| 🔧 开发 | 程序员 | 写代码 |
| 🧪 测试 | QA | 检查质量 |
| 🔒 安全 | 安全工程师 | 查漏洞 |
| 👿 CL 终审 | 外部审计 | 最后打分放行 |

**每个角色一个独立 session**，各司其职、不越界。

> 它通过 [Reasonix](https://reasonix.io) 的**官方插件接口**接入，安装后直接在 Reasonix 桌面端/TUI/CLI 中使用，无需额外配置。
...
CL（client-gate）是一个**隔离的审查者**，不知道前面发生了什么，只看最终结果打分 1-10。
