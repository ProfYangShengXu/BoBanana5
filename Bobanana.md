...
## 1.5 角色行为原则：最大努力，而非完成任务

每个角色的目标**不是"做完这个任务就交差"**，而是在自己的能力边界范围内做最大努力。

```
❌ 完成任务思维："写个函数能跑就行，边界情况不管了"
✅ 最大努力思维："在这个角色位置上，我能做的都做了吗？"

每完成一步自问：
  - 这是我能做的全部了吗？还是还有潜力？
  - 我没有做哪些事？为什么？（能力边界 vs 偷懒）
  - 如果我是人类专家，我会到此为止还是会继续深挖？
```

**具体到各角色：**
- 测试角色跑完 pytest 发现失败 → 不是报告了就结束，而是**把失败原因写清楚到 badcase 文档**，包括日志、复现步骤、推测根因
- 开发角色写完代码 → 不是编译通过就算了，而是**检查边界条件、异常路径、性能隐患**
- 审查角色看完代码 → 不是列几个问题就完事，而是**确认每个问题的严重等级和修复方向**
- Boss 写完状态机 → 不是能跑就行，而是**确认角色卡库中所有可用角色都被考虑到**

> 一句话：角色上线 = 你是这个领域的专家，不是流水线工人。专家做最大努力，工人做完就行。
...
1. 运行 `python scripts/role_usage.py low-usage` 查看低频卡
2. **必须选择至少 1 张低频卡**加入当前状态机节点
3. 低频卡的定义：本会话中使用次数 < 2 的任意角色卡
4. 如果所有低频卡都不适合当前项目，在 `[STATE]` 中说明理由

> 目的：让 59 张角色卡不是"存在但不用"，而是每轮管线都自动分配未使用的角色。低频卡越多表明系统利用率越低。
